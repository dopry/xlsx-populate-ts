...
const dateConverter = {
  ...
  dateToNumber(date) {
    ...
    let number = Math.round(
      ((dateOnly as any) - (dateBase as any)) / millisecondsInDay,
    );

    // Add the true fractional days from just the milliseconds left in the current day.
    number += ((date as any) - (dateOnly as any)) / millisecondsInDay;
  ...
  numberToDate(number) {
    ...
    const partialMilliseconds = Math.round(
      (number - fullDays) * millisecondsInDay,
    );
