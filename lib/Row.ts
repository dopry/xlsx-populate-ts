...
class Row {
  ...
  cell(columnNameOrNumber) {
    ...
    if (columnNumber < 1)
      throw new RangeError(
        `Invalid column number ${columnNumber}. Remember that spreadsheets use 1-based indexing.`,
      );
  ...
  height(...args: any[]) {
    return new ArgHandler("Row.height")
      .case(() => {
        return this._node.attributes.customHeight
          ? this._node.attributes.ht
          : undefined;
      })
      .case("number", (height) => {
