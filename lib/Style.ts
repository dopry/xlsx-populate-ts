 ...
 */
class Style {
  protected _borderNode: any;
  protected _fillNode: any;
  protected _fontNode: any;
  protected _id: any;
  protected _styleSheet: any;
  protected _xfNode: any;
  ...
  _get_justifyLastLine() {
    return (
      xmlq.getChildAttribute(this._xfNode, "alignment", "justifyLastLine") === 1
    );
  }
  ...
  _get_shrinkToFit() {
    return (
      xmlq.getChildAttribute(this._xfNode, "alignment", "shrinkToFit") === 1
    );
  }
  ...
  _get_textDirection() {
    const readingOrder = xmlq.getChildAttribute(
      this._xfNode,
      "alignment",
      "readingOrder",
    );
  ...
  _get_fill() {
    ...
    const patternType =
      patternFillNode && patternFillNode.attributes.patternType; // jq.get(patternFillNode, "$.patternType");
    ...
    _.forOwn(settings, (setting, side) => {
      ...
      if (side === "diagonal") {
        xmlq.setAttributes(this._borderNode, {
          diagonalUp:
            setting.direction === "up" || setting.direction === "both"
              ? 1
              : null,
          diagonalDown:
            setting.direction === "down" || setting.direction === "both"
              ? 1
              : null,
        });
      }
  ...
  _set_numberFormat(formatCode) {
    this._xfNode.attributes.numFmtId =
      this._styleSheet.getNumberFormatId(formatCode);
  }
}
