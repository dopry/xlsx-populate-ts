...
class Encryptor {
  ...
  encrypt(data, password) {
    ...
    const verifierHashValue = this._hash(
      encryptionInfo.key.hashAlgorithm,
      verifierHashInput,
    );
    ...
    // Create a new CFB
    let output: any = cfb.utils.cfb_new();
    ...
    if (!Buffer.isBuffer(output)) output = Buffer.from(output as any);
  ...
  _buildEncryptionInfo(encryptionInfo) {
    ...
    const encryptionInfoNode = {
      ...
      children: [
        ...
        },
        {
          ...
          attributes: {
            encryptedHmacKey:
              encryptionInfo.dataIntegrity.encryptedHmacKey.toString("base64"),
            encryptedHmacValue:
              encryptionInfo.dataIntegrity.encryptedHmacValue.toString(
                "base64",
              ),
          },
        },
              ...
              children: [
                {
                  ...
                  attributes: {
                    ...
                    encryptedVerifierHashInput:
                      encryptionInfo.key.encryptedVerifierHashInput.toString(
                        "base64",
                      ),
                    encryptedVerifierHashValue:
                      encryptionInfo.key.encryptedVerifierHashValue.toString(
                        "base64",
                      ),
                    encryptedKeyValue:
                      encryptionInfo.key.encryptedKeyValue.toString("base64"),
                  },
                },
              ],
            },
    ...
    return Buffer.concat([
      ...
      Buffer.from(encryptionInfoXml, "utf8"),
    ]);
  }
  ...
  _parseEncryptionInfoAsync(buffer) {
    ...
    return xmlParser.parseAsync(xml).then((doc) => {
      ...
      const keyEncryptorNode = xmlq.findChild(
        keyEncryptorsNode,
        "keyEncryptor",
      );
      const encryptedKeyNode = xmlq.findChild(
        keyEncryptorNode,
        "p:encryptedKey",
      );
  ...
  _crypt(encrypt, cipherAlgorithm, cipherChaining, key, iv, input) {
    ...
    const cipher = crypto[encrypt ? "createCipheriv" : "createDecipheriv"](
      algorithm,
      key,
      iv,
    );
  ...
  _cryptPackage(
    ...
    if (encrypt) {
      ...
      output = Buffer.concat([
        this._createUInt32LEBuffer(input.length, PACKAGE_OFFSET),
        output,
      ]);
    } else {
