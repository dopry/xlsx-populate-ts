 ...
 */
class Sheet {
  protected _autoFilter: any;
  protected _colBreaksNode: any;
  protected _colNodes: any;
  protected _colsNode: any;
  protected _columns: any;
  protected _dataValidations: any;
  protected _dataValidationsNode: any;
  protected _hyperlinks: any;
  protected _hyperlinksNode: any;
  protected _idNode: any;
  protected _maxSharedFormulaId: any;
  protected _mergeCells: any;
  protected _mergeCellsNode: any;
  protected _node: any;
  protected _pageBreaks: any;
  protected _pageMarginsNode: any;
  protected _pageMarginsPresetName: any;
  protected _pageMarginsPresets: any;
  protected _printOptionsNode: any;
  protected _relationships: any;
  protected _rowBreaksNode: any;
  protected _rows: any;
  protected _sheetDataNode: any;
  protected _sheetPrNode: any;
  protected _workbook: any;
  ...
  active(...args: any[]) {
    return new ArgHandler("Sheet.active")
      ...
      .case("boolean", (active) => {
        if (!active)
          throw new Error(
            "Deactivating sheet directly not supported. Activate a different sheet instead.",
          );
  ...
  activeCell(...args: any[]) {
    ...
    return new ArgHandler("Sheet.activeCell")
      .case(() => {
        const cellAddress = selectionNode
          ? selectionNode.attributes.activeCell
          : "A1";
      ...
      .case("*", (cell) => {
        ...
        selectionNode.attributes.activeCell = selectionNode.attributes.sqref =
          cell.address();
        return this;
      })
      .handle(arguments);
  ...
  column(columnNameOrNumber) {
    const columnNumber =
      typeof columnNameOrNumber === "string"
        ? addressConverter.columnNameToNumber(columnNameOrNumber)
        : columnNameOrNumber;
    ...
    if (existingColNode) {
      ...
      if (existingColNode.attributes.min < columnNumber) {
        ...
        for (
          let i = beforeColNode.attributes.min;
          i <= beforeColNode.attributes.max;
          i++
        ) {
  ...
  gridLinesVisible() {
    ...
    return new ArgHandler("Sheet.gridLinesVisible")
      .case(() => {
        return (
          sheetViewNode.attributes.showGridLines === 1 ||
          sheetViewNode.attributes.showGridLines === undefined
        );
      })
    ...
    return new ArgHandler("Sheet.range")
      ...
      .case(
        ...
        (
          ...
          return this.range(
            this.cell(startRowNumber, startColumnNameOrNumber),
            this.cell(endRowNumber, endColumnNameOrNumber),
          );
        },
      )
      .handle(arguments);
  ...
  row(rowNumber) {
    if (rowNumber < 1)
      throw new RangeError(
        `Invalid row number ${rowNumber}. Remember that spreadsheets use 1-based indexing.`,
      );
  ...
  tabColor(...args: any[]) {
    return new ArgHandler("Sheet.tabColor")
      ...
      .case("object", (color) => {
        const tabColorNode = xmlq.appendChildIfNotFound(
          this._sheetPrNode,
          "tabColor",
        );
  ...
  usedRange() {
    ...
    for (let i = 0; i < this._rows.length; i++) {
      ...
      if (
        minUsedColumnNumber > 0 &&
        (!minColumnNumber || minUsedColumnNumber < minColumnNumber)
      )
        minColumnNumber = minUsedColumnNumber;
      if (
        maxUsedColumnNumber > 0 &&
        (!maxColumnNumber || maxUsedColumnNumber > maxColumnNumber)
      )
  ...
  hyperlink(...args: any[]) {
    return new ArgHandler("Sheet.hyperlink")
      .case("string", (address) => {
        ...
        const relationship = this._relationships.findById(
          hyperlinkNode.attributes["r:id"],
        );
      ...
      .case(["string", "string", "boolean"], (address, hyperlink, internal) => {
        const isHyperlinkInternalAddress =
          internal || addressConverter.fromAddress(hyperlink);
        let nodeAttributes;
        if (isHyperlinkInternalAddress) {
          ...
          const relationship = this._relationships.add(
            "hyperlink",
            hyperlink,
            "External",
          );
      ...
      .case(["string", "object"], (address, opts) => {
        if (opts instanceof Cell) {
          ...
          this.hyperlink(
            address,
            encodeURI(`mailto:${email}?subject=${subject}`),
          );
        }
        const hyperlinkNode = this._hyperlinks[address];
        if (hyperlinkNode) {
      ...
      .case("string", (address) => {
        if (this._dataValidations[address]) {
          return {
            ...
            formula2: this._dataValidations[address].children[1]
              ? this._dataValidations[address].children[1].children[0]
              : undefined,
          };
        } else {
          return false;
  ...
  toXmls() {
    ...
    this._colsNode.children = _.filter(this._colNodes, (colNode, i) => {
      ...
      return (
        colNode &&
        i === colNode.attributes.min &&
        Object.keys(colNode.attributes).length > 2
      );
    });
  ...
  printOptions(...args: any[]) {
    ...
    const checkAttributeName = this._getCheckAttributeNameHelper(
      "printOptions",
  ...
  printGridLines() {
    return new ArgHandler("Sheet.gridLines")
      .case(() => {
        return (
          this.printOptions("gridLines") && this.printOptions("gridLinesSet")
        );
      })
      .case(["nil"], () => {
    ...
    return new ArgHandler("Sheet.pageMargins")
      ...
      .case(["string", "string"], (attributeName, attributeStringValue) => {
        return this.pageMargins(
          attributeName,
          parseFloat(attributeStringValue),
        );
      })
      .handle(arguments);
  }
    ...
    return new ArgHandler("Sheet.pageMarginsPreset")
      ...
      .case(["string", "object"], (presetName, presetAttributes) => {
        if (this._pageMarginsPresets.hasOwnProperty(presetName)) {
          throw new Error(
            `Sheet.pageMarginsPreset: The preset ${presetName} already exists!`,
          );
        }
  ...
  panes(...args: any[]) {
    ...
    const supportedActivePanes = [
      "bottomLeft",
      "bottomRight",
      "topLeft",
      "topRight",
    ];
    const checkStateName = this._getCheckAttributeNameHelper(
      "pane.state",
      supportedStates,
    );
    const checkActivePane = this._getCheckAttributeNameHelper(
      "pane.activePane",
      supportedActivePanes,
    );
    const sheetViewNode = this._getOrCreateSheetViewNode();
   ...
   * @return {Sheet} The sheet
   */
  freezePanes(...args: any[]) {
    return new ArgHandler("Sheet.feezePanes")
      .case(["integer", "integer"], (xSplit, ySplit) => {
        const topLeftCell =
          addressConverter.columnNumberToName(xSplit + 1) + (ySplit + 1);
  ...
  _getCheckTypeHelper(functionName, valueType) {
    return (value) => {
      if (typeof value !== valueType) {
        throw new TypeError(
          `Sheet.${functionName}: invalid type - value must be of type ${valueType}.`,
        );
      }
    };
  }
  ...
  _getCheckRangeHelper(functionName, valueMin, valueMax) {
    ...
    return (value) => {
      ...
      if (valueMin !== undefined) {
        if (value < valueMin) {
          throw new RangeError(
            `Sheet.${functionName}: value too small - value must be greater than or equal to ${valueMin}.`,
          );
        }
      }
      if (valueMax !== undefined) {
        if (valueMax <= value) {
          throw new RangeError(
            `Sheet.${functionName}: value too large - value must be less than ${valueMax}.`,
          );
        }
      }
    };
  ...
  _init(workbook, idNode, node, relationshipsNode) {
    ...
    dataValidationNodes.forEach((dataValidationNode) => {
      this._dataValidations[dataValidationNode.attributes.sqref] =
        dataValidationNode;
    });
