 ...
 */
class Workbook {
  protected _activeSheet: any;
  protected _appProperties: any;
  protected _contentTypes: any;
  protected _coreProperties: any;
  protected _maxSheetId: any;
  protected _node: any;
  protected _relationships: any;
  protected _sharedStrings: any;
  protected _sheets: any;
  protected _sheetsNode: any;
  protected _styleSheet: any;
  protected _zip: any;
  ...
  outputAsync(opts) {
    ...
    this._sheets.forEach((sheet, i) => {
      ...
      xmlq.appendChild(definedNamesNode, {
        ...
        children: [
          sheet._autoFilter.address({ includeSheetName: true, anchored: true }),
        ],
      });
    });
    ...
    this._zip.file(
      "[Content_Types].xml",
      xmlBuilder.build(this._contentTypes),
      zipFileOpts,
    );
    this._zip.file(
      "docProps/app.xml",
      xmlBuilder.build(this._appProperties),
      zipFileOpts,
    );
    this._zip.file(
      "docProps/core.xml",
      xmlBuilder.build(this._coreProperties),
      zipFileOpts,
    );
    this._zip.file(
      "xl/_rels/workbook.xml.rels",
      xmlBuilder.build(this._relationships),
      zipFileOpts,
    );
    this._zip.file(
      "xl/sharedStrings.xml",
      xmlBuilder.build(this._sharedStrings),
      zipFileOpts,
    );
    this._zip.file(
      "xl/styles.xml",
      xmlBuilder.build(this._styleSheet),
      zipFileOpts,
    );
    this._zip.file(
      "xl/workbook.xml",
      xmlBuilder.build(this._node),
      zipFileOpts,
    );
  ...
  toFileAsync(path, opts) {
    ...
    return this.outputAsync(opts).then(
      (data) =>
  ...
  _addSheet(name, indexOrBeforeSheet, getTemplateNodes?) {
    ...
    if (name.length > maxSheetNameLength)
      throw new Error(
        `Sheet name may not be greater than ${maxSheetNameLength} characters.`,
      );
    if (this.sheet(name))
  ...
  _initAsync(data, opts) {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        return this._convertInputToBufferAsync(data, opts.base64).then(
          (buffer) => {
            data = buffer;
          },
        );
        ...
        return Promise.all(
          this._sheetsNode.children.map((sheetIdNode, i) => {
            ...
            return this._parseNodesAsync([
              ...
              this._sheets[i] = new Sheet(
                this,
                sheetIdNode,
                sheetNode,
                sheetRelationshipsNode,
              );
            });
          }),
        );
  ...
  _parseSheetRefs() {
    ...
    const workbookViewNode =
      bookViewsNode && xmlq.findChild(bookViewsNode, "workbookView");
    const activeTabId =
      (workbookViewNode && workbookViewNode.attributes.activeTab) || 0;
  ...
  _setSheetRefs() {
    ...
    workbookViewNode.attributes.activeTab = this._sheets.indexOf(
      this._activeSheet,
    );
  ...
  _convertBufferToOutput(buffer, type) {
    ...
    if (process.browser && type === "blob")
      return new Blob([buffer], { type: Workbook.MIME_TYPE });
