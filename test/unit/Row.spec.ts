...
describe("Row", () => {
  ...
  beforeEach(() => {
    ...
    MockCell.prototype.toObject = vi.fn().mockImplementation(function (
      this: any,
    ) {
      return this.id;
    });
    ...
    sheet = createSpyObj("sheet", [
      "name",
      "workbook",
      "existingColumnStyleId",
      "forEachExistingColumnNumber",
    ...
    sheet.existingColumnStyleId.mockImplementation((columnNumber: number) =>
  ...
  describe("address", () => {
    it("should return the address", () => {
      ...
      expect(row.address({ includeSheetName: true, anchored: true })).toBe(
        "'NAME'!$7:$7",
      );
    });
  });
  ...
  describe("_init", () => {
    it("should store existing rows", () => {
      ...
      expect(MockCell).toHaveBeenCalledWith(row, {
        name: "c",
        attributes: { r: "B7" },
      });
    });
