...
const MockRange = vi.hoisted(() => vi.fn() as any);
const MockRow = vi.hoisted(() => vi.fn() as any);
const MockColumn = vi.hoisted(() => vi.fn() as any);
const MockCell = vi.hoisted(() => vi.fn() as any);
const MockRelationships = vi.hoisted(() => vi.fn() as any);

vi.mock("../../lib/Range", () => ({ default: MockRange }));
vi.mock("../../lib/Row", () => ({ default: MockRow }));
vi.mock("../../lib/Column", () => ({ default: MockColumn }));
...
describe("Sheet", () => {
  ...
  beforeEach(() => {
    ...
    MockRange.mockReset();
    MockRow.mockReset();
    MockColumn.mockReset();
    MockCell.mockReset();
    MockRelationships.mockReset();

    // MockRow: constructor and prototype methods
    MockRow.mockImplementation(function (this: any, sheet: any, node: any) {
      this._sheet = sheet;
      this._node = node;
    });
    MockRow.prototype.rowNumber = vi.fn().mockImplementation(() => ++i);
    MockRow.prototype.find = vi.fn();

    // MockColumn: constructor stores its args
    MockColumn.mockImplementation(function (this: any, sheet: any, node: any) {
      this._sheet = sheet;
      this._node = node;
    });

    // MockCell: prototype address
    MockCell.mockImplementation(function (this: any) {});
    MockCell.prototype.address = vi.fn().mockReturnValue("ADDRESS");

    // MockRelationships
    MockRelationships.mockImplementation(function (this: any, node: any) {
      this._node = node;
    });
    MockRelationships.prototype.findById = vi
      .fn()
      .mockImplementation((id: string) => ({
        attributes: { Target: `TARGET:${id}` },
      }));
    MockRelationships.prototype.add = vi
      .fn()
      .mockReturnValue({ attributes: { Id: "ID" } });

    // MockRange — basic constructor
    MockRange.mockImplementation(function (
      this: any,
      startCell: any,
      endCell: any,
    ) {
    ...
    workbook = createSpyObj("workbook", [
      "scopedDefinedName",
      "activeSheet",
      "sheets",
      "deleteSheet",
    ...
    workbook.scopedDefinedName.mockReturnValue("DEFINED NAME");
    workbook.activeSheet.mockReturnValue("ACTIVE SHEET");
  ...
  describe("active", () => {
    it("should return true/false", () => {
      ...
      workbook.activeSheet.mockReturnValue(sheet);
  ...
  describe("activeCell", () => {
    ...
    beforeEach(() => {
      cell = new MockCell();
      vi.spyOn(sheet, "cell").mockReturnValue(cell);
    });
        ...
        children: [
          {
            ...
            children: [{ name: "selection", attributes: { activeCell: "B5" } }],
          },
        ],
      });
    ...
    it("should set the active cell by cell", () => {
      ...
      expect(sheetNode.children[1]).toEqual({
        name: "sheetViews",
        attributes: {},
        children: [
          {
            ...
            children: [
              {
                name: "selection",
                attributes: { activeCell: "ADDRESS", sqref: "ADDRESS" },
                children: [],
              },
            ],
          },
  ...
  describe("cell", () => {
    ...
    beforeEach(() => {
      ...
      vi.spyOn(sheet, "row").mockReturnValue({ cell });
    });
  ...
  describe("column", () => {
    ...
    it("should create a new column", () => {
      ...
      expect(column).toBeInstanceOf(MockColumn);
      expect(sheet._columns[5]).toBe(column);
      const colNode = MockColumn.mock.calls[0][1];
      expect(colNode).toEqual({
        name: "col",
        attributes: { min: 5, max: 5 },
        children: [],
      });
      expect(MockColumn).toHaveBeenCalledWith(sheet, colNode);
      expect(sheet._colNodes[5]).toEqual(colNode);
    });

    it("should break an existing column", () => {
      const existingColNode = {
        name: "col",
        attributes: { min: 4, max: 7 },
        children: [],
      };
      ...
      expect(colNode).toEqual({
        name: "col",
        attributes: { min: 6, max: 6 },
        children: [],
      });
      expect(MockColumn).toHaveBeenCalledWith(sheet, colNode);
      expect(sheet._colNodes).toEqual([
        undefined,
        undefined,
        undefined,
        undefined,
        { name: "col", attributes: { min: 4, max: 5 }, children: [] },
        { name: "col", attributes: { min: 4, max: 5 }, children: [] },
        colNode,
        { name: "col", attributes: { min: 7, max: 7 }, children: [] },
      ]);
    });
  });
  ...
  describe("find", () => {
    it("should return the matches", () => {
      ...
      MockRow.prototype.find.mockReturnValue(["A", "B"]);
      expect(sheet.find("foo")).toEqual(["A", "B", "A", "B", "A", "B"]);
      expect(MockRow.prototype.find).toHaveBeenCalledWith(/foo/gim, undefined);

      MockRow.prototype.find.mockReturnValue("C");
      expect(sheet.find("bar", "baz")).toEqual(["C", "C", "C"]);
      expect(MockRow.prototype.find).toHaveBeenCalledWith(/bar/gim, "baz");
    });
  });
  ...
  describe("hidden", () => {
    ...
    it("should hide/unhide the sheet", () => {
      workbook.sheets.mockReturnValue([
        sheet,
    ...
    it("should hide the sheet and activate a different one", () => {
      ...
      workbook.sheets.mockReturnValue([sheet, otherSheet]);

      vi.spyOn(sheet, "active").mockReturnValue(true);
      sheet.hidden(true);
  ...
  describe("name", () => {
    ...
    it("sheet name should be a string", () => {
      idNode = {
        name: "sheet",
        attributes: { name: 1, sheetId: "1", "r:id": "rId1" },
        children: [],
      };
      sheet = new Sheet(workbook, idNode, sheetNode);
  ...
  describe("range", () => {
    beforeEach(() => {
      vi.spyOn(sheet, "cell").mockImplementation((a: any, b: any) => [a, b]);
    });

    it("should create a range from a range address", () => {
      expect(sheet.range("A2:B3")).toBeInstanceOf(MockRange);
      expect(MockRange).toHaveBeenCalledWith([2, 1], [3, 2]);
    });

    it("should create a range from two cells or addresses", () => {
      const c1 = {},
        c2 = {};
      expect(sheet.range(c1, c2)).toBeInstanceOf(MockRange);
      expect(MockRange).toHaveBeenCalledWith(c1, c2);

      expect(sheet.range("A1", c2)).toBeInstanceOf(MockRange);
      expect(MockRange).toHaveBeenCalledWith(["A1", undefined], c2);

      expect(sheet.range(c1, "C3")).toBeInstanceOf(MockRange);
      expect(MockRange).toHaveBeenCalledWith(c1, ["C3", undefined]);

      expect(sheet.range("A1", "C3")).toBeInstanceOf(MockRange);
    ...
    it("should create a range from row numbers and column names and numbers", () => {
      expect(sheet.range(1, 2, 3, 4)).toBeInstanceOf(MockRange);
      expect(MockRange).toHaveBeenCalledWith([1, 2], [3, 4]);

      expect(sheet.range(1, "B", 3, "D")).toBeInstanceOf(MockRange);
      expect(MockRange).toHaveBeenCalledWith([1, "B"], [3, "D"]);
    });
  });
  ...
  describe("autoFilter", () => {
    ...
    it("should add a XML node", () => {
      // Use a plain mock range so toXmls can call address()
      const range = { address: () => "B3:C3" };
      sheet.autoFilter(range);

      const props = sheet
        .toXmls()
        .sheet.children.filter((child: any) => child.name === "autoFilter");
  ...
  describe("row", () => {
    ...
    it("should create a new row", () => {
      ...
      expect(row).toBeInstanceOf(MockRow);
      expect(sheet._rows[5]).toBe(row);
      expect(MockRow).toHaveBeenCalledWith(sheet, {
        name: "row",
        attributes: { r: 5 },
        children: [],
      });
  ...
  describe("tabColor", () => {
    it("should get the tab color", () => {
      ...
      sheet._sheetPrNode.children = [
        { name: "tabColor", attributes: { rgb: "RGB" } },
      ];
      expect(sheet.tabColor()).toEqual({ rgb: "RGB" });

      sheet._sheetPrNode.children = [
        { name: "tabColor", attributes: { theme: 0 } },
      ];
      expect(sheet.tabColor()).toEqual({ theme: 0 });

      sheet._sheetPrNode.children = [
        { name: "tabColor", attributes: { rgb: "RGB", tint: "TINT" } },
      ];
      expect(sheet.tabColor()).toEqual({ rgb: "RGB", tint: "TINT" });

      sheet._sheetPrNode.children = [
        { name: "tabColor", attributes: { indexed: 5 } },
      ];
      expect(sheet.tabColor()).toEqual({ rgb: "FFFF00" });
    });

    it("should set the tab color", () => {
      expect(sheet.tabColor("ff0000")).toBe(sheet);
      expect(sheet._sheetPrNode.children).toEqual([
        {
          name: "tabColor",
      ...
      expect(sheet._sheetPrNode.children).toEqual([
        {
          name: "tabColor",
          attributes: { rgb: "FF0000", tint: -0.5 },
          children: [],
        },
      ]);
  ...
  describe("tabSelected", () => {
    let sheetViewNode: any;

    beforeEach(() => {
      sheetViewNode = { attributes: {} };
      vi.spyOn(sheet, "_getOrCreateSheetViewNode").mockReturnValue(
        sheetViewNode,
      );
    });
  ...
  describe("usedRange", () => {
    beforeEach(() => {
      vi.spyOn(sheet, "range").mockReturnValue("RANGE" as any);
    });
    ...
    it("should return the used range", () => {
      sheet._rows = [
        ...
        undefined,
        undefined,
        { minUsedColumnNumber: () => 3, maxUsedColumnNumber: () => 5 },
        undefined,
        undefined,
        { minUsedColumnNumber: () => 2, maxUsedColumnNumber: () => 4 },
      ];
  ...
  describe("clearCellsUsingSharedFormula", () => {
    it("should clear cells with matching shared formula", () => {
      sheet._rows = [
        undefined,
        { clearCellsUsingSharedFormula: vi.fn() },
        undefined,
        { clearCellsUsingSharedFormula: vi.fn() },
      ];

      sheet.clearCellsUsingSharedFormula(3);
      expect(sheet._rows[1].clearCellsUsingSharedFormula).toHaveBeenCalledWith(
        3,
      );
      expect(sheet._rows[3].clearCellsUsingSharedFormula).toHaveBeenCalledWith(
        3,
      );
    });
  });
  ...
  describe("existingColumnStyleId", () => {
    ...
    it("should return the style ID from the column", () => {
      sheet._colNodes[5] = { attributes: { style: "STYLE ID" } };
  ...
  describe("dataValidation", () => {
    it("should return the dataValidation Object", () => {
      ...
      expect(sheet.dataValidation("A1")).toEqual({
        type: "list",
        formula1: "test1, test2, test3",
      });
      expect(sheet.dataValidation("A2")).toEqual({
        type: "list",
        formula1: "test1, test2, test3",
      });
    ...
    it("should add a dataValidations entry", () => {
      expect(sheet._dataValidations).toEqual({});
      ...
      expect(sheet._dataValidations).toEqual({
        A1: {
          ...
          children: [
            { name: "formula1", atrributes: {}, children: ["TEST"] },
            { name: "formula2", atrributes: {}, children: [""] },
          ],
        },
      });
    });
    ...
    it("should remove a dataValidation entry", () => {
      sheet._dataValidations = { ADDRESS1: {}, ADDRESS2: {} };
      ...
      expect(sheet._dataValidations).toEqual({ ADDRESS2: {} });
  ...
  describe("hyperlink", () => {
    ...
    it("should add a hyperlink entry", () => {
      expect(sheet._hyperlinks).toEqual({});
      expect(sheet.hyperlink("ADDRESS", "HYPERLINK")).toBe(sheet);
      expect(sheet._hyperlinks).toEqual({
        ADDRESS: {
          name: "hyperlink",
          attributes: { "r:id": "ID", ref: "ADDRESS" },
          children: [],
        },
      });
    });
    ...
    it("should remove a hyperlink entry", () => {
      sheet._hyperlinks = { ADDRESS1: {}, ADDRESS2: {} };

      expect(sheet.hyperlink("ADDRESS3", undefined)).toBe(sheet);
      expect(sheet._hyperlinks).toEqual({ ADDRESS1: {}, ADDRESS2: {} });

      sheet.hyperlink("ADDRESS1", undefined);
      expect(sheet._hyperlinks).toEqual({ ADDRESS2: {} });
    ...
    it("should set the hyperlink and the tooltip on the sheet", () => {
      ...
      expect(sheet._relationships.add).toHaveBeenCalledWith(
        "hyperlink",
        "HYPERLINK",
        "External",
      );
    ...
    it("should add a hyperlink entry using opts.email and opts.emailSubject", () => {
      const opts = { email: "USER@SERVER.COM", emailSubject: "EMAIL SUBJECT" };
  ...
  describe("printOptions", () => {
    it("should return the printOptions attribute value", () => {
      sheet._printOptionsNode = {
        name: "printOptions",
        attributes: { headings: 1, horizontalCentered: 0 },
        children: [],
      };
    ...
    it("should throw an error if attempting to access unsupported attributes", () => {
      sheet._printOptionsNode = {
        name: "printOptions",
        attributes: { unsupportedAttribute: "a value of some kind" },
        children: [],
      };

      const theError =
        'Sheet.printOptions: "unsupportedAttribute" is not supported.';
      expect(() => sheet.printOptions("unsupportedAttribute")).toThrowError(
        theError,
      );
      expect(() =>
        sheet.printOptions("unsupportedAttribute", undefined),
      ).toThrowError(theError);
      expect(() =>
        sheet.printOptions("unsupportedAttribute", true),
      ).toThrowError(theError);

      const theOtherError =
        'Sheet.printOptions: "anotherUnsupportedAttribute" is not supported.';
      expect(() =>
        sheet.printOptions("anotherUnsupportedAttribute"),
      ).toThrowError(theOtherError);
      expect(() =>
        sheet.printOptions("anotherUnsupportedAttribute", undefined),
      ).toThrowError(theOtherError);
      expect(() =>
        sheet.printOptions("anotherUnsupportedAttribute", true),
      ).toThrowError(theOtherError);
    });
    ...
    it("should remove a printOptions attribute", () => {
      ...
      expect(sheet._printOptionsNode.attributes).toEqual({
        headings: 1,
  ...
  describe("printGridLines", () => {
    it("should return the combined gridLines and gridLinesSet state from printOptions", () => {
      sheet._printOptionsNode = {
        name: "printOptions",
        attributes: { headings: 1, gridLines: 1 },
        children: [],
      };
    ...
    it("should add or update the gridLines and gridLinesSet printOptions attributes", () => {
      ...
      expect(sheet._printOptionsNode.attributes).toEqual({ headings: 1 });
    });
  });
  ...
  describe("pageMargins", () => {
    ...
    it("should return the pageMargins attribute value without depending on preset", () => {
      ...
      sheet._pageMarginsNode = {
        name: "pageMargins",
        attributes: { left: 0.7, footer: "0.3" },
        children: [],
      };

      expect(sheet.pageMargins("left", "0.3")).toBe(sheet);
      expect(sheet._pageMarginsNode.attributes.left).toBe(0.3);

      expect(sheet.pageMargins("footer", 0.7)).toBe(sheet);
      expect(sheet._pageMarginsNode.attributes.footer).toBe(0.7);

      expect(sheet.pageMargins("header", 1.0)).toBe(sheet);
      expect(sheet._pageMarginsNode.attributes.header).toBe(1.0);
    });

    it("should throw an error if attempting to assign a value with an undefined preset", () => {
      sheet._pageMarginsPresetName = undefined;
      expect(() => sheet.pageMargins("left", 123)).toThrowError(
        "Sheet.pageMargins: preset is undefined.",
      );
    });
  ...
  describe("pageMarginsPreset", () => {
    ...
    it("should throw an error if accessing non-existing presets", () => {
      expect(() => sheet.pageMarginsPreset("NOT_A_PRESET_NAME")).toThrowError(
        'Sheet.pageMarginsPreset: "NOT_A_PRESET_NAME" is not supported.',
      );
    });

    it("should throw an error if a preset is defined without all necessary attributes", () => {
      expect(() =>
        sheet.pageMarginsPreset("my_preset", { left: 123 }),
      ).toThrowError(
        'Sheet.pageMarginsPreset: Invalid preset attributes for one or key(s)! - "left"',
      );
    });
  });
  ...
  describe("merged", () => {
    it("should return true/false if the cells are merged or not", () => {
      sheet._mergeCells = { ADDRESS1: {}, ADDRESS2: {} };
    ...
    it("should add a mergeCell entry", () => {
      expect(sheet._mergeCells).toEqual({});
    ...
    it("should remove the mergeCell entry", () => {
      ...
      expect(sheet._mergeCells).toEqual({ ADDRESS1: {}, ADDRESS2: {} });

      sheet.merged("ADDRESS1", false);
      expect(sheet._mergeCells).toEqual({ ADDRESS2: {} });
  ...
  describe("toXmls", () => {
    ...
    it("should add the columns", () => {
      sheet._colsNode = { name: "cols", attributes: {}, children: ["foo"] };
      sheet._colNodes = [
        null,
        { name: "col", attributes: { min: 1, max: 2, foo: true } },
        { name: "col", attributes: { min: 1, max: 2, foo: true } },
        { name: "col", attributes: { min: 3, max: 3, foo: true } },
        { name: "col", attributes: { min: 4, max: 4 } },
      ];
    ...
    describe("printOptions", () => {
      ...
      it("should add the printOptions if an attribute is defined", () => {
        sheet._printOptionsNode = {
          name: "printOptions",
          attributes: { verticalCentered: false },
          children: [],
        };
        expect(sheet.toXmls().sheet.children).toEqual([
          { name: "sheetPr", attributes: {}, children: [] },
    ...
    describe("pageMargins", () => {
      ...
      it("should not add pageMargins if preset is undefined", () => {
        ...
        sheet._pageMarginsNode = {
          name: "pageMargins",
          attributes: { left: 1, right: 2, top: 3, bottom: 4, header: 5 },
          children: [],
        };
        expect(sheet.pageMarginsPreset()).toBe("template");
    ...
    it("should add the mergeCells", () => {
      sheet._mergeCells = { "A1:B2": "MERGE1", "C3:D4": "MERGE2" };
      expect(sheet.toXmls().sheet.children).toEqual([
        { name: "sheetPr", attributes: {}, children: [] },
        { name: "sheetFormatPr", attributes: {}, children: [] },
        { name: "sheetData", attributes: {}, children: [] },
        { name: "mergeCells", attributes: {}, children: ["MERGE1", "MERGE2"] },
      ]);
    });

    it("should add the hyperlinks", () => {
      sheet._hyperlinks = { A1: "HYPERLINK1", C3: "HYPERLINK2" };
      expect(sheet.toXmls().sheet.children).toEqual([
        { name: "sheetPr", attributes: {}, children: [] },
        { name: "sheetFormatPr", attributes: {}, children: [] },
        { name: "sheetData", attributes: {}, children: [] },
    ...
    it("should add the hyperlinks and merge cells in the proper order", () => {
      ...
      expect(sheet.toXmls().sheet.children).toEqual([
        ...
        { name: "sheetFormatPr", attributes: {}, children: [] },
        { name: "sheetData", attributes: {}, children: [] },
        { name: "mergeCells", attributes: {}, children: ["MERGE1"] },
        {
        ...
        { name: "hyperlinks", attributes: {}, children: ["HYPERLINK1"] },
      ]);
    });
  });
  ...
  describe("_init", () => {
    it("should create the sheet node", () => {
      sheet._init({}, {});
      expect(sheet._node).toEqual(expect.any(Object));
    });
    ...
    it("should create the relationships", () => {
      sheet._init(
        ...
        {
          attributes: {},
          children: [{ name: "sheetData", attributes: {}, children: [] }],
        },
        "RELATIONSHIPS",
      );

      expect(sheet._relationships).toBeInstanceOf(MockRelationships);
      expect(MockRelationships).toHaveBeenCalledWith("RELATIONSHIPS");
    });
    ...
    it("should delete the optional dimension node", () => {
      ...
      expect(sheet._node.children).toEqual([
        { name: "sheetPr", attributes: {}, children: [] },
        { name: "sheetData", attributes: {}, children: [] },
      ]);
    });
    ...
    it("should parse the rows", () => {
      ...
      expect(MockRow).toHaveBeenCalledWith(sheet, "ROW1");
      expect(MockRow).toHaveBeenCalledWith(sheet, "ROW2");
      expect(MockRow).toHaveBeenCalledWith(sheet, "ROW3");
    });
    ...
    it("should create the sheetPr node", () => {
      ...
      expect(sheet._sheetPrNode).toEqual({
        name: "sheetPr",
        attributes: {},
        children: [],
      });
