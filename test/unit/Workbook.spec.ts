...
describe("Workbook", () => {
    ...
    const resolved = (val: any) =>
        new Promise<void>(resolve => setTimeout(resolve, Math.random() * 10)).then(() => val);
    ...
    beforeEach(() => {
        ...
        MockJSZip.prototype.file = vi.fn().mockImplementation((fileName: string) => ({
        ...
        MockFs.readFile.mockImplementation((_path: string, cb: Function) => cb(null, "DATA"));
        MockFs.writeFile.mockImplementation((_path: string, _data: any, cb: Function) => cb(null));
        ...
        // Sheet
        MockSheet.mockClear();
        MockSheet.mockImplementation(function (this: any, workbook: any, sheetIdNode: any, sheetNode: any, sheetRelationshipsNode: any) {
        ...
        MockXmlParser.prototype.parseAsync = vi.fn().mockImplementation((text: string) =>
            Promise.resolve(`JSON(${text})`)
        );

        // XmlBuilder
        MockXmlBuilder.mockClear();
        MockXmlBuilder.prototype.build = vi.fn().mockImplementation((obj: any) =>
            `XML: ${obj && obj.toString()}`
        );

        // Encryptor
        MockEncryptor.mockClear();
        MockEncryptor.prototype.encrypt = vi.fn().mockImplementation((input: any) => `ENCRYPTED(${input})`);
        MockEncryptor.prototype.decryptAsync = vi.fn().mockImplementation((input: any) =>
    ...
    describe("static", () => {
        beforeEach(() => {
            vi.spyOn(Workbook.prototype as any, "_initAsync").mockResolvedValue("WORKBOOK");
        });
        ...
        describe("fromBlankAsync", () => {
            it("should init with blank data", async () => {
                ...
                expect((Workbook.prototype as any)._initAsync).toHaveBeenCalledWith("BLANK", undefined);
        ...
        describe("fromDataAsync", () => {
            it("should init with the data", async () => {
                ...
                expect((Workbook.prototype as any)._initAsync).toHaveBeenCalledWith("DATA", "OPTS");
                expect(wb).toBe("WORKBOOK");
            });
        });
        ...
        describe("fromFileAsync", () => {
            it("should init with the file data", async () => {
                ...
                expect((Workbook.prototype as any)._initAsync).toHaveBeenCalledWith("DATA", "OPTS");
                expect(MockFs.readFile).toHaveBeenCalledWith("PATH", expect.any(Function));
                expect(wb).toBe("WORKBOOK");
            });
        });
    });
    ...
    describe("instance", () => {
        ...
        describe("addSheet", () => {
            beforeEach(() => {
                ...
                workbook._relationships.add.mockReturnValue({ attributes: { Id: 'RID' } });
        ...
        describe("definedName", () => {
            it("should return the scoped defined name", () => {
                vi.spyOn(workbook, 'scopedDefinedName').mockReturnValue("SCOPED DEFINED NAME");
        ...
        describe("find", () => {
            it("should return the matches", () => {
                ...
                expect(MockSheet.prototype.find).toHaveBeenCalledWith(/foo/gim, undefined);
        ...
        describe("outputAsync", () => {
            ...
            it("should output the data", async () => {
                ...
                expect(workbook._zip.generateAsync).toHaveBeenCalledWith({ type: 'nodebuffer', compression: "DEFLATE" });
            ...
            it("should encrypt the workbook if password is set", async () => {
                ...
                expect(MockEncryptor.prototype.encrypt).toHaveBeenCalledWith("ZIP", "PASSWORD");
        ...
        describe("toFileAsync", () => {
            it("should write the workbook to file", async () => {
                ...
                expect(MockFs.writeFile).toHaveBeenCalledWith("PATH", "OUTPUT", expect.any(Function));
            });
        });
        ...
        describe("_initAsync", () => {
            beforeEach(() => {
                vi.spyOn(workbook, "_parseNodesAsync").mockImplementation((files: string[]) => {
                    return Promise.all(_.map(files, (file: string) => {
                        if (file === "xl/workbook.xml") return resolved(workbookNode);
                        return resolved(`PARSED(${file})`);
                    }));
                });
                vi.spyOn(workbook, "_parseSheetRefs");
                vi.spyOn(workbook, "_convertInputToBufferAsync").mockResolvedValue("BUFFER");
            });
            ...
            it("should extract the files from the data zip and load the objects", async () => {
                ...
                expect(MockContentTypes).toHaveBeenCalledWith('PARSED([Content_Types].xml)');
                expect(MockRelationships).toHaveBeenCalledWith('PARSED(xl/_rels/workbook.xml.rels)');
                expect(MockSharedStrings).toHaveBeenCalledWith('PARSED(xl/sharedStrings.xml)');
                expect(MockStyleSheet).toHaveBeenCalledWith('PARSED(xl/styles.xml)');

                expect(MockRelationships.prototype.findByType).toHaveBeenCalledWith('sharedStrings');
                expect(MockContentTypes.prototype.findByPartName).toHaveBeenCalledWith("/xl/sharedStrings.xml");
            ...
            it("should decrypt the data if a password is set", async () => {
                ...
                expect(MockEncryptor.prototype.decryptAsync).toHaveBeenCalledWith("BUFFER", "PASSWORD");
            ...
            it("should add the shared strings if not present", async () => {
                ...
                await workbook._initAsync("DATA");
                expect(MockRelationships.prototype.add).toHaveBeenCalledWith("sharedStrings", "sharedStrings.xml");
                expect(MockContentTypes.prototype.add).toHaveBeenCalledWith("/xl/sharedStrings.xml", "application/vnd.openxmlformats-officedocument.spreadsheetml.sharedStrings+xml");
            });
        });
        ...
        describe("_parseNodesAsync", () => {
            it("should parse the nodes", async () => {
                ...
                expect(MockXmlParser.prototype.parseAsync).toHaveBeenCalledWith('TEXT(foo)');
                expect(MockXmlParser.prototype.parseAsync).toHaveBeenCalledWith('TEXT(bar)');
                expect(MockXmlParser.prototype.parseAsync).toHaveBeenCalledWith('TEXT(baz)');
        ...
        describe("_parseSheetRefs", () => {
            ...
            it("should parse the active sheet", () => {
                workbook._node.children = [{
                    name: "bookViews",
                    attributes: {},
                    children: [{ name: "workbookView", attributes: { activeTab: 1 }, children: [] }]
                }];
            ...
            it("should parse the defined names sheets", () => {
                workbook._node.children = [{
                    ...
                    attributes: {},
                    children: [
                        { name: "definedName", attributes: { name: "WORKBOOK_SCOPE" }, children: ["VALUE1"] },
                        { name: "definedName", attributes: { name: "SHEET_SCOPE", localSheetId: 0 }, children: ["VALUE2"] }
                    ]
                }];
                ...
                expect(workbook._node.children).toEqual([{
                    ...
                    children: [
                        { name: "definedName", attributes: { name: "WORKBOOK_SCOPE" }, children: ["VALUE1"] },
                        { name: "definedName", attributes: { name: "SHEET_SCOPE", localSheetId: 0 }, children: ["VALUE2"], localSheet: "SHEET1" }
                    ]
                }]);
            });
        });
        ...
        describe("_setSheetRefs", () => {
            ...
            it("should set the active sheet and use the existing book view", () => {
                workbook._node.children = [{ name: "bookViews", attributes: { foo: true }, children: [] }];
            ...
            it("should set the defined names sheets", () => {
                workbook._node.children = [{
                    ...
                    children: [
                        { name: "definedName", attributes: { name: "WORKBOOK_SCOPE" }, children: ["VALUE1"] },
                        { name: "definedName", attributes: { name: "SHEET_SCOPE" }, children: ["VALUE2"], localSheet: "SHEET1" }
                    ]
                }];
        ...
        describe("_convertInputToBufferAsync", () => {
            ...
            it("should convert a base64 string", async () => {
                const output = await workbook._convertInputToBufferAsync("Zm9vYmFy", true);
                expect(Buffer.isBuffer(output)).toBe(true);
                expect(output).toEqual(Buffer.from([0x66, 0x6f, 0x6f, 0x62, 0x61, 0x72]));
            });
