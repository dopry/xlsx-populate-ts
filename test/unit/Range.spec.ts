...
describe("Range", () => {
  ...
  beforeEach(() => {
    ...
    Style.prototype.style = vi
      .fn()
      .mockImplementation((name: string) => `STYLE:${name}`);
    style = new Style();

    sheet = createSpyObj("sheet", [
      "name",
      "workbook",
      "cell",
      "merged",
    ...
    sheet.cell.mockImplementation(
      (row: number, column: number) => `CELL[${row}, ${column}]`,
    );
    sheet.workbook.mockReturnValue("WORKBOOK");
    sheet.dataValidation.mockReturnValue("DATAVALIDATION");

    startCell = createSpyObj("startCell", [
      "rowNumber",
      "columnNumber",
      "columnName",
      "sheet",
  ...
  describe("formula", () => {
    ...
    it("should set the shared formula", () => {
      ...
      expect(cells["3, 2"].setSharedFormula).toHaveBeenCalledWith(
        8,
        "FORMULA",
        "B3:C5",
      );
  ...
  describe("map", () => {
    it("should call the callback for each cell and return the values", () => {
      const callback = vi
        .fn()
        .mockImplementation(
          (cell: any, ri: number, ci: number) => `RETURN[${ri}, ${ci}]`,
        );
  ...
  describe("reduce", () => {
    it("should call the callback for each cell and return the aggregate value", () => {
      const callback = vi
        .fn()
        .mockImplementation(
          (accumulator: string, cell: any, ri: number, ci: number) =>
            `${accumulator} RETURN[${ri}, ${ci}]`,
        );
      expect(range.reduce(callback, "INITIAL")).toBe(
        "INITIAL RETURN[0, 0] RETURN[0, 1] RETURN[1, 0] RETURN[1, 1] RETURN[2, 0] RETURN[2, 1]",
      );
      ...
      expect(callback.mock.calls[2]).toEqual([
        "INITIAL RETURN[0, 0] RETURN[0, 1]",
        "CELL[4, 2]",
        1,
        0,
        range,
      ]);
      expect(callback.mock.calls[3]).toEqual([
        "INITIAL RETURN[0, 0] RETURN[0, 1] RETURN[1, 0]",
        "CELL[4, 3]",
        1,
        1,
        range,
      ]);
      expect(callback.mock.calls[4]).toEqual([
        "INITIAL RETURN[0, 0] RETURN[0, 1] RETURN[1, 0] RETURN[1, 1]",
        "CELL[5, 2]",
        2,
        0,
        range,
      ]);
      expect(callback.mock.calls[5]).toEqual([
        "INITIAL RETURN[0, 0] RETURN[0, 1] RETURN[1, 0] RETURN[1, 1] RETURN[2, 0]",
        "CELL[5, 3]",
        2,
        1,
        range,
  ...
  describe("dataValidation", () => {
    it("should return the range", () => {
      ...
      expect(sheet.dataValidation).toHaveBeenCalledWith(
        "B3:C5",
        "testing, testing2",
      );
    });
