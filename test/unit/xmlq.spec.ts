...
describe("xmlq", () => {
    describe("appendChild", () => {
        it("should append the child", () => {
            ...
            expect(node).toEqual({ name: 'parent', children: [{ name: 'existing' }, { name: 'new' }] });
        });
        ...
        it("should create the children array if needed", () => {
            ...
            const child = { name: 'new' };
            xmlq.appendChild(node, child);
            expect(node).toEqual({ name: 'parent', children: [{ name: 'new' }] });
        });
    });
    ...
    describe("appendChildIfNotFound", () => {
        it("should append the child", () => {
            const node = { name: 'parent', children: [{ name: 'existing' }] };
            expect(xmlq.appendChildIfNotFound(node, 'new')).toEqual({ name: 'new', attributes: {}, children: [] });
            expect(node).toEqual({ name: 'parent', children: [{ name: 'existing' }, { name: 'new', attributes: {}, children: [] }] });
        });

        it("should not append the child", () => {
            const node = { name: 'parent', children: [{ name: 'existing' }] };
            expect(xmlq.appendChildIfNotFound(node, 'existing')).toEqual({ name: 'existing' });
            expect(node).toEqual({ name: 'parent', children: [{ name: 'existing' }] });
        });
    });
    ...
    describe("insertAfter", () => {
        it("should insert the child after the node", () => {
            ...
            expect(node.children).toEqual([{ name: 'A' }, { name: 'new' }, { name: 'B' }]);
        });
    });
    ...
    describe("insertInOrder", () => {
        it("should insert in the middle", () => {
            ...
            expect(node.children).toEqual([{ name: 'A' }, { name: 'B' }, { name: 'C' }]);
        });

        it("should insert at the beginning", () => {
            const node = { name: 'parent', children: [{ name: 'C' }] };
            xmlq.insertInOrder(node, { name: 'A' }, ['A', 'B', 'C']);
            expect(node.children).toEqual([{ name: 'A' }, { name: 'C' }]);
        });
        ...
        it("append if node not expected in order", () => {
            ...
            expect(node.children).toEqual([{ name: 'A' }, { name: 'C' }, { name: 'D' }]);
        });
    });
    ...
    describe("removeChild", () => {
        it("should remove the children", () => {
            ...
            expect(node.children).toEqual([{ name: 'C' }]);
    ...
    describe("setChildAttributes", () => {
        it("should append the child with the attributes", () => {
            const node = { name: 'parent', children: [{ name: 'existing' }] };
            expect(xmlq.setChildAttributes(node, 'new', { foo: 1, bar: null })).toEqual({ name: 'new', attributes: { foo: 1 }, children: [] });
            expect(node.children).toEqual([{ name: 'existing' }, { name: 'new', attributes: { foo: 1 }, children: [] }]);
        });
        ...
        it("should not append the child but should set the attributes", () => {
            ...
            expect(xmlq.setChildAttributes(node, 'existing', { foo: 1, bar: null })).toEqual({ name: 'existing', attributes: { foo: 1 } });
            expect(node).toEqual({ name: 'parent', children: [{ name: 'existing', attributes: { foo: 1 } }] });
        });
    });
    ...
    describe("removeChildIfEmpty", () => {
        it("should remove the children", () => {
            ...
            expect(node.children).toEqual([{ name: 'A' }, { name: 'C', attributes: { foo: 1 } }]);
            xmlq.removeChildIfEmpty(node, 'A');
            expect(node.children).toEqual([{ name: 'C', attributes: { foo: 1 } }]);
