...
describe("Column", () => {
  ...
  beforeEach(() => {
    ...
    verticalPageBreaks = createSpyObj("verticalPageBreaks", ["add"]);
    ...
    existingRows = [
      ...
      },
      {
        hasStyle: () => true,
        hasCell: () => false,
        _cell: createSpyObj("cell[1]", ["style"]),
        cell: vi.fn().mockImplementation(function (this: any) {
          return this._cell;
        }),
      },
      {
        hasStyle: () => false,
        hasCell: () => true,
        _cell: createSpyObj("cell[2]", ["style"]),
        cell: vi.fn().mockImplementation(function (this: any) {
          return this._cell;
        }),
      },
  ...
  describe("address", () => {
    it("should return the address", () => {
      ...
      expect(column.address({ includeSheetName: true, anchored: true })).toBe(
        "'NAME'!$E:$E",
      );
    });
  });
  ...
  describe("style", () => {
    beforeEach(() => {
      vi.spyOn(column, "_createStyleIfNeeded");
      column._style = style;
    });
    ...
    it("should get multiple styles", () => {
      expect(column.style(["foo", "bar", "baz"])).toEqual({
        foo: "STYLE:foo",
        bar: "STYLE:bar",
        baz: "STYLE:baz",
      });
